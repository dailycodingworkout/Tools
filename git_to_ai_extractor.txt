...
class GitToAIExtractor:
    ...
    def get_commit_changes(self, commit_hash: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # For merge commits, use different approach
            if is_merge_commit:
                files_changed = self.run_git_command([
                    "diff-tree", "--no-commit-id", "--name-status", "-r", "-m", commit_hash
                ])
            else:
                # For regular commits, check if it's the initial commit
                if not parents:
            ...
            # If still no files, try alternative approach
